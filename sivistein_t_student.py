        ...
        for epoch in tqdm(range(1, self.trainpara.num_epochs+1)):
            ...
            if (epoch - 1)%self.config.sampling.visual_time ==0 or epoch ==self.trainpara.num_epochs:
                with torch.no_grad():
                    X = self.SemiVInet.sampling(num = self.config.sampling.num)
                    torch.save(X,os.path.join(self.save_fig_to_path, str(self.iter_idx)+'.pt'))
                if self.target in ["banana", "multimodal", "x_shaped","student_uc"]:
                    save_to_path = os.path.join(self.save_fig_to_path, str(self.iter_idx)+'.png')
                    bbox = [-4,4,-4,4]
                    quiver_plot = False
                    self.target_model.contour_plot(bbox, fnet = None, samples=X.cpu().numpy(), save_to_path=save_to_path, quiver = quiver_plot, t = self.iter_idx)
            ...
            for i in range(1, self.trainpara.num_perepoch+1):
                ...
                if self.trainpara.affine_invariant:
                    with torch.no_grad():
                        inv_cov_matrix = torch.inverse(torch.cov(X.T).to(self.device)+1e-8 * torch.eye(X.shape[-1]).to(self.device))
                    x_xaux_dist = torch.matmul((compu_targetscore + neg_score_implicit) @ inv_cov_matrix,(compu_targetscore_aux  + neg_score_implicit_aux).T)
                else:
                ...
                if self.iter_idx < 20000:
                    loss_kxy = (x_xaux_dist * kernel_dist).mean() - 0.05 * (self.target_model.logp(X)).mean() * annealing_coef(self.iter_idx)
                else:
